...
export async function GET(
  ...
  try {
    ...
    const isParticipant = chat.user1_id === session.userId || chat.user2_id === session.userId;
...
export async function POST(
  ...
  try {
    ...
    if (voice_url) messageData.voice_url = voice_url;
...
export async function PATCH(
  ...
  try {
    ...
    if (action === 'edit' && text) {
      if (!isSender) return NextResponse.json({ error: 'Нет доступа' }, { status: 403 });
